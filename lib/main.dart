...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _exportPdf() async {
    try {
      final pdf = pw.Document();
      final DateFormat df = DateFormat('yyyy-MM-dd HH:mm'); // Updated to include time
      ...
      pdf.addPage(
        pw.MultiPage(
          build: (context) => [
            ...
            pw.Paragraph(text: 'Generated: ${df.format(DateTime.now())}'),
            pw.Table.fromTextArray(
              headers: ['Type', 'Amount', 'Note', 'Date & Time'], // Updated headers, removed category
              data: _all
                  .map((t) => [
                t.type,
                t.amount.toStringAsFixed(2),
                        ...
                        child: filtered.isEmpty
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return TransactionTile(
                              t: t,
                              onTap: () => _openAdd(t),
                              onEdit: () => _openAdd(t), // Added edit callback
                              onDelete: () => _delete(t),
                              currency: currency,
                              isCompact: isMobile,
                            );
